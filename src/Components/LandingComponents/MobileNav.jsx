...
const MobileNav = ({ openNav }) => {
  return (
    <nav className={`sm:hidden fixed flex flex-col align-middle p-8 gap-8 h-screen w-screen bg-white dark:bg-black z-50 ${openNav ? 'open-nav' : 'close-nav'}`}>
      ...
      <Link to="/signup" className="flex justify-center items-center align-middle px-8 py-3 bg-[#6C3BAA] text-white font-semibold rounded-full hover:cursor-pointer hover:opacity-85">Sign Up</Link>
    </nav>
  )
}
